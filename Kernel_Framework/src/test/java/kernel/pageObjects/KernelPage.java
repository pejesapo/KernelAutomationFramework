...
public abstract class KernelPage<T> {
	...
	public T openPage(Class<T> clazz) {
		...
		ExpectedCondition pageLoadCondition = ((KernelPage) page).getPageLoadCondition();
		logger.info("Waiting for {} page to get fully loaded...", clazz.toString() );
	...
	private void waitForPageToLoad(ExpectedCondition pageLoadCondition) {
		Wait wait = new FluentWait(this.browser).withTimeout(LOAD_TIMEOUT,
				TimeUnit.SECONDS).pollingEvery(REFRESH_RATE, TimeUnit.SECONDS).ignoring(NoSuchElementException.class);;
